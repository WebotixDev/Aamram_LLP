...
@section('css')
    ...
    <style>
/*  #sale-order-main-table th:nth-child(1)
{
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(2)
 {
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(3)
 {
    min-width: 60px;
    max-width: 60px;
}
#sale-order-main-table th:nth-child(4) {
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(5){
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(6) {
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(7) {
    min-width: 80px;
    max-width: 80px;
}*/
...
#sale-order-main-table th:nth-child(8) {
    min-width: 90px;
    max-width: 90px;
}
#sale-order-main-table th:nth-child(9) {
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(10) {
    min-width: 80px;
    max-width: 80px;
}
#sale-order-main-table th:nth-child(11) {
    min-width: 470px;
    max-width: 470px;
}
...
<div class="container-fluid basic_table">
    <div class="page-title">
        <div class="row">
            <div class="col-sm-6">
                <h4> Sale Order  <a href="{{ route('admin.sale_order.create') }}" class="btn btn-secondary">{{ __('Add') }}</a></h4>
            </div>
...
<script>
    ...
    function fetchTableData(from_date = null, to_date = null, user_id = null) {
        if ($.fn.DataTable.isDataTable('#sale-order-main-table')) {
            $('#sale-order-main-table').DataTable().destroy();
        }

        $('#sale-order-main-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route("admin.sale_order.data") }}',
                data: {
                    from_date: from_date,
                    to_date: to_date,
                    user_id: user_id
            ...
            // ],
             ...
             columns: [
                { data: 'sr_no', name: 'sr_no', orderable: true, searchable: true },
                { data: 'billdate', name: 'billdate', orderable: true, searchable: true },
                { data: 'customer_name', name: 'customer_display_name' ,orderable: true, searchable: true},
                { data: 'product_name', name: 'product_name' ,orderable: true, searchable: true },
                { data: 'size_name', name: 'size_name', orderable: true, searchable: true },
                { data: 'stage', name: 'stage' ,orderable: true, searchable: true},
                { data: 'qty', name: 'qty' ,orderable: true, searchable: true},
                // { data: 'rate', name: 'rate' },
                { data: 'action', name: 'action', orderable: false, searchable: false },
                { data: 'amount', name: 'amount', orderable: true, searchable: true },
                { data: 'Tamount', name: 'Tamount' ,orderable: true, searchable: true},
                { data: 'order_address', name: 'order_address', orderable: true, searchable: true },
    ...
    $(document).ready(function () {
        ...
        $('#labour').click(function () {
            const from_date = $('#from_date').val();
            const to_date = $('#to_date').val();
            let url = `{{ route('admin.Bulk_labour-bill-print') }}?from_date=${from_date}&to_date=${to_date}`;
...
<!--        if ($.fn.dataTable.isDataTable('#sale-order-main-table')) {-->
<!--            $('#sale-order-main-table').DataTable().clear().destroy();-->
<!--        }-->
<!--        $('#sale-order-main-table').DataTable({-->
<!--    processing: true,-->
<!--    serverSide: true,-->
...
<!--    columns: [-->
<!--        { data: 'sr_no', name: 'sr_no' },-->
<!--        { data: 'billdate', name: 'billdate',  orderable: true, searchable: false },-->
...
<!--        { data: 'Tamount', name: 'Tamount', orderable: true, searchable: false },-->
<!--        { data: 'action', name: 'action', orderable: false, searchable: false },-->
<!--    ],-->
