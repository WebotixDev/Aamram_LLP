            ...
            <div class="col-md-12">
                <table id="saleTable" style="width:100%; font-size:12px" class="table table-bordered">
                    ...
                    <tbody id="p_scents">
                            ...
                            @php
                            $sizes = DB::table('product_details')->where('sizeoff', 0)->where('disable','!=',1)->where('parentID', $sale_order_list1['services'])->get();
                            @endphp
...
//                     let size = $(this).val();
//                     let stage = $("#stage_" + rowIndex).val(); // Get the selected stage
...
//                     let stage = $(this).val();
//                     let size = $("#size_" + rowIndex).val(); // Get the selected size
...
function fetchrate(id) {
    ...
    let services = $("#services_" + rowIndex).val();  // Selected Product
    let size = $("#size_" + rowIndex).val();          // Selected Size

    if (!services || !size) {
        return; // Exit if product or size is not selected
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === "success") {
                let rate = response.rate || 0;
                $("#rate_" + rowIndex).val(rate);
                Gettotal();  // Trigger Gettotal() after updating rate
