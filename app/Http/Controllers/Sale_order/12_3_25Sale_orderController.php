...
public function getPrice(Request $request) {
    ...
    $productprice = Product_details::where('parentID', $product->id)->first(['id', 'dist_price']); // Fetch the price for the first item
...
public function getRate(Request $request)
{
    ...
    public function edit(sale_order $saleOrder)
    {

        $saleOrders = sale_details::where('id',$saleOrder->id)->first();
        $sale_order_list = sale_order::where('pid',$saleOrders->id)->get();
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(Request $request, sale_order $saleOrder)
    {
        return $this->repository->update($request->all(), $saleOrder->id);
    }
...
public function BulklabourBillPrint(Request $request)
{
    ...
    $saleOrders = DB::table('sale_orderdetails')
        ...
        ->leftJoin('cities', 'customers.city_name', '=', 'cities.id')
        ->select(
            'sale_order.pid',
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function destroy(Request $request, sale_order $saleOrder)
    {
        return $this->repository->destroy($saleOrder->id);
    }
}
